...
const getItemsFilter = function (type) {
  ...
  switch (type) {
    ...
    case "done":
      ...
      break;
    case "sortByDate":
      filterItems = todoItems.sort(
        (a, b) => new Date(b.date) - new Date(a.date)
      );
      break;
    case "sortByName":
      filterItems = todoItems.sort((a, b) => a.name - b.name);
      break;
    default:
  ...
  items.forEach((item) => {
    if (
      ...
      item.querySelector("[data-edit]").addEventListener("click", function (e) {
        ...
        document.querySelector("#item").value = todoItems.indexOf(itemData);
        return todoItems;
      });
...
document.addEventListener("DOMContentLoaded", () => {
  form.addEventListener("submit", (e) => {
    ...
    if (itemName.length === 0) {
      ...
      const currentItemIndex = document.querySelector("#item").value;
      if (currentItemIndex) {
